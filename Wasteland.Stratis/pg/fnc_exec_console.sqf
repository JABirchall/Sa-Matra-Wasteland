...
switch (_0ta) do {
	...
	case 1: {//exec
		...
		_0ma = call compile _command;
		if (!(isNil {_0ma})) then {
			GET_CTRL(balca_debug_console_result_IDC) ctrlSetText str _0ma;
			__uiSet(balca_console_result,_0ma);
		};
	};
...
/*		[] spawn {
			_0d = time+2;
			waitUntil{
				((count(player getVariable ['PG_result',[]])==1)||_0d>time)
			};
	...
	case 6: {//run tracker
		...
		_0uu = compile _command;
		_0ma = _0uu spawn {while {PG_tracker} do {sleep .5;call _this}};
		if (!(isNil {_0ma})) then {
			GET_CTRL(balca_debug_console_result_IDC) ctrlSetText str _0ma;
			__uiSet(balca_console_result,_0ma);
		};
	...
	case 123: {
		...
		if(balca_debug_console_expand) then {
			_05j = _pos_result select 3;

			_pos_result set [3, _05j * 4];

			_pos_history_title set [1, (_pos_history_title select 1) + _05j * (EXPAND_TIMES - 1)];

			_pos_history set [1, (_pos_history select 1) + _05j * (EXPAND_TIMES - 1)];
			_pos_history set [3, (_pos_history select 3) - _05j * (EXPAND_TIMES - 1)];
		} else {
			_05j = (_pos_result select 3) / EXPAND_TIMES;

			_pos_result set [3, _05j];
