...
case 0: {//init
	...
	_lb = parseText "<br/>";
	_0dn = getText(configFile >> "cfgWeapons" >> _0dg >> "displayName");
	...
	_0pr = composeText [_mode_text,
		"	Weapon: ",_0dn,_lb,
		"Shots: ",str _shots,_lb,
		"Hits: ", str _hits,_lb,
		"Kills: ", str _kills,_lb,
		"Accuracy: ", str (_hits/(_shots max 1)),_lb];
	GET_CTRL(balca_stat_text_IDC) ctrlSetStructuredText _0pr;
};
case 1: {//reset
...
case 4: {//hit EH
	_0li = (_opt select 1);
	if ((_0li == (vehicle player))&&((_stat select 0) == currentWeapon(vehicle player))) then {
