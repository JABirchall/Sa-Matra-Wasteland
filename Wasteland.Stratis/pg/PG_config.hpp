...
/*			class balca_targets_btn : balca_debug_btn
			...
			class balca_realign_core_btn : balca_debug_btn
			{
				...
				y = btn_height*6;
				text = "Realign core";
				action = "closeDialog 0;_core = c_proving_ground_core;_05a = direction player;_pos = getPos player;_core setPos [(_pos select 0)+10*sin(_05a),(_pos select 1)+10*cos(_05a),0];_core setDir _05a;_0vd = createMarkerLocal ['respawn_west',_pos];createMarkerLocal ['respawn_east',_pos];createMarkerLocal ['respawn_guerrila',_pos];createMarkerLocal ['respawn_civilian',_pos];closeDialog 0;"; 
			};*/
		...
		//column 2
			class balca_camtp_btn : balca_debug_btn
			{
				...
				y = btn_height*0;
				text = "TP to camera";
				action = "_01c = positionCameraToWorld [0,0,0]; if(surfaceIsWater _01c) then {player setPosASL _01c} else {player setPosATL _01c}"; 
			};
...
/*			class balca_ammo_btn : balca_debug_btn
			...
			class balca_teleport_btn : balca_debug_btn
			{
				...
				y = btn_height*3;
				text = "TP";
				action = "hint ""Click on map to TP"";onMapSingleClick ""vehicle player setPos [_pos select 0,_pos select 1,0]; onMapSingleClick '';""; if(!visibleMap) then {openMap true;}; closeDialog 0;"; 
			};
