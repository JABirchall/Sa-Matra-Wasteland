...
_drawMarkers = {
	...
	{if ((waypointType _x)=="MOVE") then {
		...
		_0pr = str _forEachIndex + " " + str (waypointType _x);
		[_markerName,_pos,_0pr] call _addMarker;
...
case 0: {//init;
		_target_mode = if isNil{_opt} then {//restore values
			...
			PG_set(target_mode,_opt);
			{
				_0cf = _x select 0;
				deleteVehicle _0cf;
				if !(_0cf isKindOf "Man") then {{deleteVehicle _x} forEach units (_x select 2);};
...
case 1: {
		...
		for "_i" from 0 to (count _cfgvehicles)-1 do {
			...
			if (isClass _0fd) then {
				_opt = configName(_0fd);
				if ((getNumber(_0fd >> "scope")==2)and(KINDOF_ARRAY(_opt,_kindOf))and!(KINDOF_ARRAY(_opt,_filter))) then {
					GET_CTRL(balca_target_vehlist_IDC) lbAdd (getText(_0fd >> "displayName"));
					...
					if !(_kindOf select 0 in ["TargetBase","CAManBase"]) then {
						GET_CTRL(balca_target_vehlist_IDC) lbSetPicture [(lbSize GET_CTRL(balca_target_vehlist_IDC))-1,getText(_0fd >> "picture")];
					}else{
						GET_CTRL(balca_target_vehlist_IDC) lbSetPicture [(lbSize GET_CTRL(balca_target_vehlist_IDC))-1,getText(_0fd >> "icon")];
					};
...
case 2: {//info
		...
		_0fd = (configFile >> "CfgVehicles" >> _opt);
		...
		_0pr = composeText [str configName(_0fd),_lb,
			...
			"Weapons: ", str _0o,_lb];
		GET_CTRL(balca_target_veh_info_IDC) ctrlSetStructuredText _0pr;
	};
case 3: {//add target
		...
		switch PG_get(target_mode) do {
			case 0: {//land static
				...
				if ((PG_get(TARGET_PROPS) select 1)>0) then {//if speed>0 start moving
					[] spawn {
						_04a = 0;
						_delay = 0.03;
						_05k = (PG_get(TARGET_PROPS) select 1);
						_shift_inc = (_05k*_delay);
						while {((PG_get(TARGET_PROPS) select 1) == _05k)&&(PG_get(target_mode) in [0,3])} do {
							sleep _delay;
							_04a = _04a + _shift_inc;
							if (abs(_04a)>20) then {_shift_inc = -_shift_inc};
							[_04a,-1] call PG_get(fnc_move_land_targets);
						};
			...
			case 2: {//land AI
				...
				if ((ctrlVisible balca_target_map_IDC)&&((count waypoints PG_get(TARGET_GRP))>1)) then {
					//_0gf = _grp addWaypoint [[_pos select 0,_pos select 1],0];
					//_0gf setWaypointType "CYCLE";
					[7] call PG_get(FNC_target);
				}else{
					[] call _clearMarkers;
					{deleteWaypoint [_grp,0]} forEach (waypoints _grp);
					_0gf = _grp addWaypoint [[(_pos select 0)+20*sin(_05a+90)+(_props select 0)*sin(_05a),(_pos select 1)+20*cos(_05a+90)+(_props select 0)*cos(_05a)],0,0];
					_0gf setWaypointType "MOVE";
					_0gf setWaypointCompletionRadius 30;
					_0gf = _grp addWaypoint [[(_pos select 0)-20*sin(_05a+90)+(_props select 0)*sin(_05a),(_pos select 1)-20*cos(_05a+90)+(_props select 0)*cos(_05a)],0,1];
					_0gf setWaypointType "MOVE";
					_0gf setWaypointCompletionRadius 30;
					_0gf = _grp addWaypoint [[_pos select 0,_pos select 1],0,2];
					_0gf setWaypointType "CYCLE";
				};
				[] call _reset_land;
			};
			case 3: {//air AI
				...
				if ((ctrlVisible balca_target_map_IDC)&&((count waypoints PG_get(AIR_TARGET_GRP))>1)) then {
					...
					PG_set(air_wp_dist,_air_wp_dist);
					[] call _clearMarkers;
					_0gf = _grp addWaypoint [[(_pos select 0)+_air_wp_dist,(_pos select 1)],0,0];
					_0gf setWaypointType "MOVE";
					_0gf setWaypointCompletionRadius 30;
					_0gf = _grp addWaypoint [[(_pos select 0),(_pos select 1)+_air_wp_dist],0,1];
					_0gf setWaypointType "MOVE";
					_0gf setWaypointCompletionRadius 30;
					_0gf = _grp addWaypoint [[(_pos select 0)-_air_wp_dist,(_pos select 1)],0,2];
					_0gf setWaypointType "MOVE";
					_0gf setWaypointCompletionRadius 30;
					_0gf = _grp addWaypoint [[(_pos select 0),(_pos select 1)-_air_wp_dist],0,3];
					_0gf setWaypointType "MOVE";
					_0gf setWaypointCompletionRadius 30;
					_0gf = _grp addWaypoint [[_pos select 0,_pos select 1],0,4];
					_0gf setWaypointType "CYCLE";
				};
			};
			default {};
...
case 9: {//add waypoint
		...
		_03e = (count (waypoints _grp)) max 1;
		deleteWaypoint [_grp,_03e-1];
		_0gf = _grp addWaypoint [_opt,0,_03e-1];
		_0gf setWaypointType "MOVE";
		_0gf setWaypointCompletionRadius 30;
		if (ctrlVisible balca_target_map_IDC) then {
			_markerName = format ["PG_WPMarker%1",str (_03e-1)];
			_pos = waypointPosition _0gf;
			_0pr = str (_03e-1) + " " + str (waypointType _0gf);
			[_markerName,_pos,_0pr] call _addMarker
		};
		_0gf = _grp addWaypoint [_opt,0,_03e];
		_0gf setWaypointType "CYCLE";
	};
case 10: {//unload
		[] call _clearMarkers;
