...
if PG_get(Autoheal) then {
	...
	[] spawn {
		while {PG_get(Autoheal)} do {
			...
			_veh_hit_EH = _0ca addEventHandler ["hit",{vehicle player setDammage 0}];
			_veh_dam_EH = _0ca addEventHandler ["dammaged",{vehicle player setDammage 0}];
			while {(_0ca == (vehicle player))&&PG_get(Autoheal)} do {
			...
			_0ca removeEventHandler ["hit",_veh_hit_EH];
			_0ca removeEventHandler ["dammaged",_veh_dam_EH];
		};
	};
};